...
def test_perform_forecasting_disables_cross_channel_when_checkpoint_has_no_weights(caplog, patch_external_dependencies):
    ...
    assert any("cross-channel weights are absent" in rec.getMessage() for rec in caplog.records)
...
def test_perform_forecasting_enables_cross_channel_when_checkpoint_has_weights(
    ...
    build_kwargs = patch_external_dependencies[0]
    assert build_kwargs["use_cross_channel"] is True
    assert any("cross-channel weights are present" in rec.getMessage() for rec in caplog.records)


def test_load_cached_model_reuses_checkpoint_model_across_requested_modes(monkeypatch, patch_external_dependencies):
    load_calls = 0

    def load_checkpoint(*args: object, **kwargs):
        nonlocal load_calls
        load_calls += 1
        return {}
...
def test_load_cached_model_rejects_missing_cross_channel_weights(monkeypatch):
    class IncompleteCrossChannelModel(DummyModel):
        def load_state_dict(self, state: dict, strict: bool = False):
    ...
    monkeypatch.setattr(
        forecasting,
        "build_model",
        lambda *, forecast_horizon, **kwargs: IncompleteCrossChannelModel(model_horizon=forecast_horizon),
    )
    monkeypatch.setattr(
        torch, "load", lambda *args, **kwargs: {"cross_channel_attn.attn.in_proj_weight": torch.empty(1)}
    )

    with pytest.raises(RuntimeError, match="Missing checkpoint keys for forecasting model"):
